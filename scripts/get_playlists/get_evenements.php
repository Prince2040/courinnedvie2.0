<?php
    ...
    $playlistId_evenements = 'PL9Ro4LotudZs27ERDDGgiTpdft_6qy47K';

    // Chemin du fichier cache
    $cacheFolder = dirname(dirname(__DIR__)) . "/cache/";
    $cacheFile = $cacheFolder . "data_evenements.json";
    ...
    if (file_exists($cacheFile) && time() - filemtime($cacheFile) < $cacheTime) {
        ...
        $videos_evenements_data = json_decode(file_get_contents($cacheFile), true);
    } else {
        // URL de l'API YouTube Data v3 pour récupérer les vidéos de la chaîne
        $url_evenements = "https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&playlistId=" . $playlistId_evenements . "&key=" . $apiKey;

        // Utiliser cURL pour effectuer la requête à l'API
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $url_evenements);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        $response_evenements = curl_exec($curl);
        curl_close($curl);

        // Vérifier si la requête a réussi
        if ($response_evenements) {

            $data_evenements = json_decode($response_evenements, true);

            $videos_evenements = $data_evenements['items'];

            $videos_evenements_data = [];
    
    
            foreach ($videos_evenements as $video) {
               ...
               /*echo  "<pre>" ;
                ...
                $videos_evenements_data[] = $video_evenements_data;
    
                //$videos_data[] = $video_data;
            }
    
            // Enregistrer les données dans un fichier JSON
            file_put_contents($cacheFile, json_encode($videos_evenements_data));
