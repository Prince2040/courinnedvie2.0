<?php
    ...
    $playlistId = 'PL9Ro4LotudZuy5EEma_HYWnGgc8TCEmmw';

    // Chemin du fichier cache
    $cacheFolder = dirname(dirname(__DIR__)) . "/cache/";
    $cacheFile = $cacheFolder . "data_egliseAlaUne.json";
    ...
    if (file_exists($cacheFile) && time() - filemtime($cacheFile) < $cacheTime) {
        ...
        $videos_egliseAlaUne_data = json_decode(file_get_contents($cacheFile), true);
    } else {
        ...
        if ($response) {
            ...
            $data_egliseAlaUne = json_decode($response, true);

            $videos_egliseAlaUne = $data_egliseAlaUne['items'];

            $videos_egliseAlaUne_data = [];
    
    
            foreach ($videos_egliseAlaUne as $video) {
               ...
               /*echo  "<pre>" ;
                ...
                $videos_egliseAlaUne_data[] = $video_egliseAlaUne_data;
    
                //$videos_data[] = $video_data;
            }
    
            // Enregistrer les données dans un fichier JSON
            file_put_contents($cacheFile, json_encode($videos_egliseAlaUne_data));
