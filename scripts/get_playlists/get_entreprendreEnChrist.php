<?php
    ...
    $playlistId_EntreprendreEnChrist = 'PL9Ro4LotudZvLQIirETlFdsc-m3QAPKl_';

    // Chemin du fichier cache
    $cacheFolder = dirname(dirname(__DIR__)) . "/cache/";
    $cacheFile = $cacheFolder . "data_EntreprendreEnChrist.json";
    ...
    if (file_exists($cacheFile) && time() - filemtime($cacheFile) < $cacheTime) {
        ...
        $videos_EntreprendreEnChrist_data = json_decode(file_get_contents($cacheFile), true);
    } else {
        // URL de l'API YouTube Data v3 pour récupérer les vidéos de la chaîne
        $url_EntreprendreEnChrist = "https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&playlistId=" . $playlistId_EntreprendreEnChrist . "&key=" . $apiKey;

        // Utiliser cURL pour effectuer la requête à l'API
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $url_EntreprendreEnChrist);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        $response_EntreprendreEnChrist = curl_exec($curl);
        curl_close($curl);

        // Vérifier si la requête a réussi
        if ($response_EntreprendreEnChrist) {

            $data_EntreprendreEnChrist = json_decode($response_EntreprendreEnChrist, true);

            $videos_EntreprendreEnChrist = $data_EntreprendreEnChrist['items'];

            $videos_EntreprendreEnChrist_data = [];
    
    
            foreach ($videos_EntreprendreEnChrist as $video) {
               ...
               /*echo  "<pre>" ;
                ...
                $video_EntreprendreEnChrist_data = [
                ...
                $videos_EntreprendreEnChrist_data[] = $video_EntreprendreEnChrist_data;
    
                //$videos_data[] = $video_data;
            }
    
            // Enregistrer les données dans un fichier JSON
            file_put_contents($cacheFile, json_encode($videos_EntreprendreEnChrist_data));
