<?php
    ...
    $playlistId_eclairageSurActu = 'PL9Ro4LotudZuATxxxshCYGEc_CY56hV9L';

    // Chemin du fichier cache
    $cacheFolder = dirname(dirname(__DIR__)) . "/cache/";
    $cacheFile = $cacheFolder . "data_eclairageSurActu.json";
    ...
    if (file_exists($cacheFile) && time() - filemtime($cacheFile) < $cacheTime) {
        ...
        $videos_eclairageSurActu_data = json_decode(file_get_contents($cacheFile), true);
    } else {
        // URL de l'API YouTube Data v3 pour récupérer les vidéos de la chaîne
        $url_eclairageSurActu = "https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&playlistId=" . $playlistId_eclairageSurActu . "&key=" . $apiKey;

        // Utiliser cURL pour effectuer la requête à l'API
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $url_eclairageSurActu);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        $response_eclairageSurActu = curl_exec($curl);
        curl_close($curl);

        // Vérifier si la requête a réussi
        if ($response_eclairageSurActu) {

            $data_eclairageSurActu = json_decode($response_eclairageSurActu, true);

            $videos_eclairageSurActu = $data_eclairageSurActu['items'];

            $videos_eclairageSurActu_data = [];
    
    
            foreach ($videos_eclairageSurActu as $video) {
               ...
               /*echo  "<pre>" ;
                ...
                $videos_eclairageSurActu_data[] = $video_eclairageSurActu_data;
    
                //$videos_data[] = $video_data;
            }
    
            // Enregistrer les données dans un fichier JSON
            file_put_contents($cacheFile, json_encode($videos_eclairageSurActu_data));
