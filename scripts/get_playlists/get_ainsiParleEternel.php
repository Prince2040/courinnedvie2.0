...
$playlistIdAinsiParleEternel = 'PL9Ro4LotudZu471G-BaFwHklMPSlrywaU';

    // Chemin du fichier cache
    $cacheFolder = dirname(dirname(__DIR__)) . "/cache/";
    $cacheFile = $cacheFolder . "data_AinsiParleEternel.json";
    ...
    if (file_exists($cacheFile) && time() - filemtime($cacheFile) < $cacheTime) {
        ...
        $videos_AinsiParleEternel_data = json_decode(file_get_contents($cacheFile), true);
    } else {
        // URL de l'API YouTube Data v3 pour récupérer les vidéos de la chaîne
        $url_AinsiParleEternel = "https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&playlistId=" . $playlistIdAinsiParleEternel . "&key=" . $apiKey;

        // Utiliser cURL pour effectuer la requête à l'API
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $url_AinsiParleEternel);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        $response_AinsiParleEternel = curl_exec($curl);
        curl_close($curl);

        // Vérifier si la requête a réussi
        if ($response_AinsiParleEternel) {

            $data_AinsiParleEternel = json_decode($response_AinsiParleEternel, true);

            $videos_AinsiParleEternel = $data_AinsiParleEternel['items'];

            $videos_AinsiParleEternel_data = [];
    
    
            foreach ($videos_AinsiParleEternel as $video) {
               ...
               /*echo  "<pre>" ;
                ...
                $videos_AinsiParleEternel_data[] = $video_AinsiParleEternel_data;
    
                //$videos_data[] = $video_data;
            }
    
            // Enregistrer les données dans un fichier JSON
            file_put_contents($cacheFile, json_encode($videos_AinsiParleEternel_data));
