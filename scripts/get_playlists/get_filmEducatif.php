<?php
    ...
    $playlistId_filmEducatif = 'PL9Ro4LotudZtbAdMZCTXG0eOUAOqLhayc';

    // Chemin du fichier cache
    $cacheFolder = dirname(dirname(__DIR__)) . "/cache/";
    $cacheFile = $cacheFolder . "data_filmEducatif.json";
    ...
    if (file_exists($cacheFile) && time() - filemtime($cacheFile) < $cacheTime) {
        ...
        $videos_filmEducatif_data = json_decode(file_get_contents($cacheFile), true);
    } else {
        // URL de l'API YouTube Data v3 pour récupérer les vidéos de la chaîne
        $url_filmEducatif = "https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&playlistId=" . $playlistId_filmEducatif . "&key=" . $apiKey;

        // Utiliser cURL pour effectuer la requête à l'API
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $url_filmEducatif);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        $response_filmEducatif = curl_exec($curl);
        curl_close($curl);

        // Vérifier si la requête a réussi
        if ($response_filmEducatif) {

            $data_filmEducatif = json_decode($response_filmEducatif, true);

            $videos_filmEducatif = $data_filmEducatif['items'];

            $videos_filmEducatif_data = [];
    
    
            foreach ($videos_filmEducatif as $video) {
               ...
               /*echo  "<pre>" ;
                ...
                $videos_filmEducatif_data[] = $video_filmEducatif_data;
    
                //$videos_data[] = $video_data;
            }
    
            // Enregistrer les données dans un fichier JSON
            file_put_contents($cacheFile, json_encode($videos_filmEducatif_data));
